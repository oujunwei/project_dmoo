...
namespace sel
{
	...
	class MOEAD{
		public:
				void initializeNeighborhood(std::vector<std::vector<int>>& neighborhood,std::vector<std::vector<double>> lambda);

				void updatePopulation(CPopulationMO &pop,std::vector<std::vector<int>> neighborhood,std::vector<std::vector<double>> lambda,int type);

				double fitnessFunction(int type,std::vector<double> f,std::vector<double> lambda,std::vector<double>&idealPoint);
		

				void initializeIdealPoint(std::vector<double> &idea,CPopulationMO pop);

				void updateIdealPoint(std::vector<double> &idea,std::vector<double>f);

				void updateNeighborhood(CPopulationMO &pop,int subId,std::vector<std::vector<int>> neighborhood,std::vector<std::vector<double>> lambda,CIndividualMO child,std::vector<double> idealPoint,int type);
	};
...
//!\brief first  distribution based on weight selection strategy
class SCrowd4
{
	protected:
	double	mF,				//!< setp length
			mCR;			//!< crossover probability
	//std::vector<std::vector<double>> points;  //产生的向量
...
public:
	...
	//!\return	population
	CPopulationMO& SelectSort(CPopulationMO& pop, unsigned int size);
    CIndividualMO Generate_Individual(CPopulationMO& pop, CIndividualMO& ind, unsigned int parent_index1, unsigned int parent_index2,double F);
	CPopulationMO environmental(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int mPopSize,std::vector<std::vector<double>> points);
	CPopulationMO Select(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int mPopSize,std::vector<std::vector<double>> points);

	
	
	
	void Set(double f, double cr);
	void Swap(std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int index,unsigned int index1_indivi);
	unsigned int distance_subPop(CPopulationMO pop,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int index,unsigned int mPopSize)
;
	void individual_orientation(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int mPopSize,std::vector<std::vector<double>> points);

	unsigned int best_individual(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,unsigned int numb);

	unsigned int best_individual1(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,unsigned int numb);

	unsigned int individual_worse_distribution(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int index,unsigned int mPopSize);
	void _judge_individual_index (unsigned int index);

	double F_self_adaption(std::vector<std::vector<double>> points, unsigned int index, unsigned int numb1, unsigned int numb2);

    void individual_orientation7(CPopulationMO& pop,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number,unsigned int mPopSize,std::vector<std::vector<double>> points);
	...
	void InitDBEA(CPopulationMO& Population, unsigned int mPopSize,std::vector<std::vector<double>>& points,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number);//静态方法

	void InitDBEA_Sel(CPopulationMO& Population, unsigned int mPopSize,std::vector<std::vector<double>>& points,std::vector<std::vector<int>>& subPop_index,std::vector<int>& subPop_index_number);


	void _normal(const size_t component,const size_t division,std::vector<double>&point,std::vector<std::vector<double>>& points);
	double distance(std::vector<double> &objective,std::vector<double> &referencepoint);
	void getSubPop(CPopulationMO& Population,std::vector<std::vector<int>> &subPop_index,std::vector<std::vector<double>> points,int psize,int fsize);
};//class SCrowd3
