...
protected:
	bool gSort;                     //!< whether the population has been grid-sorted
	...
	double popDegree;					//!< the changing degree of apopulation <2016.4.14>
public:
	//!\brief	constructor
	...
	inline bool IsGridSort() {return gSort;}
	//!\brief	set the sort state
	//!\param	sort new state
	//!\return	success if it has been sorted
	inline bool IsSort(bool sort ) {mbSort=sort; return mbSort;}
	inline bool IsGridSort(bool sort ) {gSort=sort; return gSort;}
	//!\brief	get the population size
	//!\return	population size
	inline unsigned int	Size() {return (unsigned int)mvPop.size();}

	inline void degreeAssign(double degree){popDegree = degree;}
	inline double degreeReturn(){return popDegree;}
